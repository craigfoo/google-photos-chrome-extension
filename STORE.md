...
2. **Privacy** tab:
   ...
   - **Permission justifications**, one per line item:
     ...
     - Optional host permission `<all_urls>`: The extension needs to download the bytes of the image the user chose. Access is requested per image host, only at the moment of the first upload from that host, via chrome.permissions.request; nothing is accessed without that explicit grant.
