...
2. **Privacy** tab:
   ...
   - **Permission justifications**, one per line item:
     ...
     - Optional host permission `<all_urls>`: The extension needs to download the bytes of the image the user chose. Access is requested per image host (for large image CDNs that spread files across many numbered servers, such as fbcdn.net or googleusercontent.com, once for the CDN domain), only at the moment of the first upload from that host, via chrome.permissions.request; nothing is accessed without that explicit grant. On Instagram, Threads and Facebook, which cover photos with an overlay, the request names their image CDNs (cdninstagram.com, fbcdn.net) instead, for the same single purpose.
...
| Store review rejected for "excessive permissions" / "broad host permissions" | Reviewer wants the `<all_urls>` optional permission explained | Reply with the optional-host-permission justification from step 6.2, pointing out it is optional and requested per host at click time. |
